...
export default function RelatorioPDF(): JSX.Element {
  ...
  async function gerarRelatorio(): Promise<void> {
    ...
    try {
      ...
      doc.setFontSize(12)
      doc.text(
        `Resumo: ${resumo.numSalas} salas · ${resumo.numAres} ares-condicionados · ${formatCurrency(resumo.gastoMensalTotal)}/mês · ${formatNumber(resumo.litrosDiaTotal)} L/dia de água condensada`,
        14,
        33
      )
      ...
      const linhasEnergia = salas.flatMap((sala) =>
        sala.ares.map((ac) => {
      ...
      const linhasAgua = salas.flatMap((sala) =>
        sala.ares.map((ac) => {
  ...
  return (
    <div className="max-w-xl space-y-4 rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
      <h2 className="text-base font-semibold">Relatório completo em PDF</h2>
      <p className="text-sm text-gray-500 dark:text-gray-400">
        Inclui dados de salas e ares-condicionados, dimensionamento, consumo de energia, água condensada estimada,
        recomendações de troca e as fontes técnicas utilizadas nos cálculos.
      </p>
      <button
