...
export const MODELOS_COMERCIAIS_BTU = [7000, 9000, 12000, 18000, 24000, 30000, 36000, 48000, 60000] as const
...
export function calcularDetalhamentoBtu(f: FatoresBtu): DetalhamentoBtu {
...
export function calcularBtuNecessario(f: FatoresBtu): number {
  return calcularDetalhamentoBtu(f).btuNecessarioComercial
}

export function calcularConsumoEnergia(potenciaW: number, horasDia: number, diasMes: number, tarifaKwh: number) {
 ...
 */
export function calcularAguaCondensada(btuInstalado: number, umidadeRelativa: number, horasDia: number, diasMes: number) {
...
export function calcularStatusDimensionamento(btuInstaladoTotal: number, btuNecessario: number): {
  percentual: number
  status: StatusDimensionamento
} {
  if (btuNecessario <= 0) return { percentual: 0, status: 'subdimensionado' }
  const percentual = (btuInstaladoTotal / btuNecessario) * 100
  const status: StatusDimensionamento = percentual < 85 ? 'subdimensionado' : percentual > 120 ? 'superdimensionado' : 'adequado'
...
export function sugerirTrocas(salas: SalaParaTroca[]): SugestaoTroca[] {
